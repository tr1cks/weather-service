...
import com.github.tr1cks.weather.core.util.QuartzUtil;
...
import com.github.tr1cks.weather.core.jobs.WeatherServicePollingJob;
...
public class WeatherClientsService implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private void initServiceClientsJobs() {
        ...
        QuartzUtil.pauseJobGroup(scheduler, WEATHER_SERVICE_CLIENT_JOB_GROUP);
        ...
        for(WeatherService serviceSettings : weatherServices) {
            ...
            JobDetail serviceClientJobDetail = QuartzUtil.createJobDetail(serviceClientJob, "run", jobName,
                                                               WEATHER_SERVICE_CLIENT_JOB_GROUP);
            Trigger serviceClientTrigger = QuartzUtil.createSimpleTrigger(serviceSettings.getDelayInMilliseconds(),
                                                               getTriggerName(serviceSettings),
                                                               WEATHER_SERVICE_CLIENT_TRIGGER_GROUP);
            QuartzUtil.scheduleJob(scheduler, serviceClientJobDetail, serviceClientTrigger);
            if(serviceSettings.getActive()) {
                QuartzUtil.resumeJob(scheduler, jobName, WEATHER_SERVICE_CLIENT_JOB_GROUP);
            }
        }
    ...
    private void createSyncSettingsJob(long delayInMilliseconds) {
        JobDetail serviceSettingsJob = QuartzUtil.createJobDetail(this, "syncServiceSettings", SYNC_SERVICE_SETTINGS + "Job");
        Trigger serviceSettingsTrigger = QuartzUtil.createSimpleTrigger(delayInMilliseconds, SYNC_SERVICE_SETTINGS + "Trigger");

        QuartzUtil.scheduleJob(scheduler, serviceSettingsJob, serviceSettingsTrigger);
    }
    ...
    public void syncServiceSettings() {
        ...
        for(WeatherService foundServiceSettings : foundServicesSettings) {
            ...
            if(!foundServiceSettings.getDelayInMilliseconds().equals(currentJobSettings.getDelayInMilliseconds())) {
                ...
                QuartzUtil.rescheduleJob(scheduler, triggerName, WEATHER_SERVICE_CLIENT_TRIGGER_GROUP, newDelayInMs);
    ...
    private void changeJobActiveState(WeatherService foundServiceSettings, String jobName) {
        if(!foundServiceSettings.getActive()) {
            QuartzUtil.pauseJob(scheduler, jobName, WEATHER_SERVICE_CLIENT_JOB_GROUP);
        } else {
            QuartzUtil.resumeJob(scheduler, jobName, WEATHER_SERVICE_CLIENT_JOB_GROUP);
        }
    }
