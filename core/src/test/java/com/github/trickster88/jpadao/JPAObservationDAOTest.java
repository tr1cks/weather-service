...
public class JPAObservationDAOTest extends AbstractJPADAOTest {
    ...
    private Observation createLondonObservation(Date dateTime) {
        ...
        observation.setCity(notNull(cityDAO.findByName("London", "Great Britain")));
        observation.setWeatherService(notNull(weatherServiceDAO.findByName(OpenWeatherMapServiceClient.SERVICE_NAME)));
    ...
    private Observation createMoscowObservation(Date dateTime) {
        ...
        observation.setDateTime(dateTime);
        observation.setCity(notNull(cityDAO.findByName("Moscow", "Russia")));
        observation.setWeatherService(notNull(weatherServiceDAO.findByName(OpenWeatherMapServiceClient.SERVICE_NAME)));
